...
func RegisterTools(server *mcp.Server, docsService *docs.Service, driveService *drive.Service) {
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, input readDocumentInput) (*mcp.CallToolResult, struct{}, error) {
		return readDocument(ctx, docsService, input)
	})

	mcp.AddTool(server, &mcp.Tool{
		Name:        "list_documents",
		Description: "Google Drive 内の Google Docs ドキュメント一覧を取得する",
	}, func(ctx context.Context, _ *mcp.CallToolRequest, input listDocumentsInput) (*mcp.CallToolResult, struct{}, error) {
		return listDocuments(ctx, driveService, input)
	})

	mcp.AddTool(server, &mcp.Tool{
		Name:        "search_documents",
		Description: "キーワードで Google Docs ドキュメントを検索する",
	}, func(ctx context.Context, _ *mcp.CallToolRequest, input searchDocumentsInput) (*mcp.CallToolResult, struct{}, error) {
		return searchDocuments(ctx, driveService, input)
	})

	mcp.AddTool(server, &mcp.Tool{
		Name:        "get_document_info",
		Description: "Google Docs ドキュメントのメタ情報（タイトル、作成日、更新日、オーナー等）を取得する",
	}, func(ctx context.Context, _ *mcp.CallToolRequest, input getDocumentInfoInput) (*mcp.CallToolResult, struct{}, error) {
		return getDocumentInfo(ctx, driveService, input)
	})
